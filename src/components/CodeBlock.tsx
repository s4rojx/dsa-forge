...
export default function CodeBlock({ code, language = "java" }: CodeBlockProps) {
    ...
    return (<div className="rounded-xl border border-border bg-code-bg overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border/60 bg-bg-card">
        ...
        <button onClick={handleCopy} className={`flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[11px] font-mono transition-all ${copied
      ...
      <div className="overflow-x-auto py-3">
        <table className="w-full border-collapse font-mono text-[13px] leading-[1.75]">
          <tbody>
            {lines.map((line, idx) => (<tr key={idx} className="hover:bg-white/[0.02]">
                <td className="select-none px-4 text-right align-top text-[11px] text-text-3/25 w-[3ch] leading-[1.75]">
                  {idx + 1}
                </td>
                <td className="px-4 align-top whitespace-pre text-[#d4d0c8] leading-[1.75]" dangerouslySetInnerHTML={{ __html: line || " " }}/>
              </tr>))}
          </tbody>
        </table>
      </div>
