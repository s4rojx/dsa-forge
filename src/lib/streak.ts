...
export function getCurrentStreak(days: StreakDayRecord[]) {
    const activeDays = new Set(days.filter((day) => day.count > 0).map((day) => formatUtcDay(startOfUtcDay(day.date))));
...
export function getStreakWarning(days: StreakDayRecord[]) {
    ...
    const today = startOfUtcDay(new Date());
    const yesterday = addUtcDays(today, -1);
    return (!activeDays.has(formatUtcDay(today)) && activeDays.has(formatUtcDay(yesterday)));
}
export function buildHeatmapDays(days: StreakDayRecord[], weeks = 52) {
    const countsByDay = new Map(days.map((day) => [formatUtcDay(startOfUtcDay(day.date)), day.count] as const));
