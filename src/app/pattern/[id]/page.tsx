...
import { ArrowLeft, BookOpen, Check, Clock, ExternalLink, HardDrive, Lightbulb, Star, } from "lucide-react";
...
export default function PatternPage() {
    ...
    const activeTemplate = selectedLanguage === "java" ? pattern.templateCode : pattern.cppTemplate;
    return (<AppShell>
      <Link href={`/topic/${topic.slug}`} className="mb-6 inline-flex items-center gap-1.5 text-xs font-mono text-text-3 transition-colors hover:text-primary">
        ...
        <div className="space-y-5 lg:col-span-1">
          <div className="rounded-xl border border-border bg-bg-card p-5">
            ...
            <div className="space-y-3">
              {pattern.recognitionTips.map((tip, index) => (<div key={index} className="flex items-start gap-2.5">
          ...
          <div className="relative overflow-hidden rounded-xl border border-border bg-bg-card p-5">
            ...
            <div className="space-y-3">
              {pattern.proTips.map((tip, index) => (<div key={index} className="flex items-start gap-2.5">
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <div>
            ...
            return (<div key={problem.id} className={`group flex items-center gap-3 rounded-lg border px-4 py-3 transition-all ${completed
                    ...
                    <button onClick={() => toggleProblem(problem.id)} className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border transition-all ${completed
                    ...
                    <div className="min-w-0 flex-1">
                      <span className={`block truncate text-sm font-medium ${completed ? "text-success line-through" : "text-text-1"}`}>
                        {problem.title}
                      </span>
                    ...
                    <a href={problem.url} target="_blank" rel="noopener noreferrer" className="shrink-0 rounded-md p-1.5 text-text-3 transition-colors hover:bg-primary/10 hover:text-primary">
