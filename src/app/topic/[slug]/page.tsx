...
import { ArrowLeft, ChevronDown, ChevronRight, ExternalLink, Check, Lightbulb, BookOpen, Code2, Building2, Clock, HardDrive, Star, Layers, } from "lucide-react";
...
export default function TopicPage() {
    ...
    const allCompanies = useMemo(() => {
        ...
        activePattern.problems.forEach((p) => p.companies.forEach((c) => {
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <aside className="lg:w-64 shrink-0">
          <div className="sticky top-24">
            <button className="lg:hidden flex items-center gap-2 w-full rounded-lg border border-border bg-bg-card px-4 py-3 text-sm font-jakarta font-bold text-text-1 mb-4" onClick={() => setExpandedSidebar(!expandedSidebar)}>
            ...
            return (<button key={pattern.id} onClick={() => {
                    setActivePatternId(pattern.id);
                    setActiveTab("overview");
                    setExpandedSidebar(false);
                }} className={`w-full flex items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm transition-all ${isActive
                    ...
                    {progress.percentage === 100 && (<Check className="h-4 w-4 text-success shrink-0"/>)}
                  </button>);
        })}
            </div>
          </div>
        ...
        <main className="flex-1 min-w-0">
          ...
          <div className="flex gap-1 rounded-lg bg-bg-card p-1 border border-border mb-6 overflow-x-auto">
            {tabs.map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-1.5 rounded-md px-4 py-2 text-sm font-jakarta whitespace-nowrap transition-all ${activeTab === tab.id
          ...
          {activeTab === "overview" && (<div className="space-y-5">
              ...
              <div className="rounded-xl border border-border bg-bg-card p-5">
                ...
                <div className="space-y-3 pl-[38px]">
                  {activePattern.recognitionTips.map((tip, idx) => (<div key={idx} className="flex items-start gap-3">
              ...
              <div className="rounded-xl border border-border bg-bg-card p-5 relative overflow-hidden">
                ...
                <div className="space-y-3 pl-[38px]">
                  {activePattern.proTips.map((tip, idx) => (<div key={idx} className="flex items-start gap-3 group">
            ...
            ].map((stat) => (<div key={stat.label} className="rounded-xl border border-border bg-bg-card p-4 text-center">
          ...
          {activeTab === "problems" && (<div>
              ...
              <div className="flex flex-wrap items-center gap-3 mb-4">
                <div className="flex gap-1 rounded-lg bg-bg-card p-1 border border-border">
                  {["all", "easy", "medium", "hard"].map((d) => (<button key={d} onClick={() => setDifficultyFilter(d)} className={`rounded-md px-3 py-1 text-xs font-mono capitalize transition-all ${difficultyFilter === d
                    ? "bg-primary/10 text-primary"
                    : "text-text-2 hover:text-text-1"}`}>
                      {d}
                    </button>))}
                </div>
                <button onClick={() => setShowStandardOnly(!showStandardOnly)} className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-mono transition-all ${showStandardOnly
              ...
              <div className="space-y-1">
                ...
                return (<div key={problem.id} className={`group flex items-center gap-3 rounded-lg border px-4 py-3 transition-all ${completed
                        ? "border-success/20 bg-success/5"
                        : "border-border bg-bg-card hover:border-primary/20 hover:bg-bg-elevated"}`}>
                      <button onClick={() => toggleProblem(problem.id)} className={`shrink-0 flex h-5 w-5 items-center justify-center rounded-full border transition-all ${completed
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {problem.isStandard && (<Star className="h-3 w-3 text-primary shrink-0 fill-primary"/>)}
                        </div>
                      ...
                      <div className="hidden sm:flex items-center gap-1.5 shrink-0">
                        {problem.companies.slice(0, 3).map((c) => (<span key={c} className="text-[10px] font-mono text-text-3/60">
                            {c}
                          </span>))}
                      </div>

                      <a href={problem.url} target="_blank" rel="noopener noreferrer" className="shrink-0 rounded-md p-1.5 text-text-3 hover:text-primary hover:bg-primary/10 transition-colors" onClick={(e) => e.stopPropagation()}>
              ...
              {filteredProblems.length === 0 && (<div className="text-center py-10 text-sm text-text-2">
                  No problems match the selected filters.
                </div>)}
            </div>)}

          {activeTab === "template" && (<div className="space-y-4">
              <div className="flex items-center justify-between gap-3">
                ...
                return (<button key={option.id} onClick={() => setSelectedLanguage(option.id as "java" | "cpp")} className={`rounded-md px-4 py-2 text-sm font-jakarta transition-all ${isActive
                        ? "bg-primary/10 text-primary"
              ...
              <CodeBlock code={selectedLanguage === "java"
                ? activePattern.templateCode
                : activePattern.cppTemplate} language={selectedLanguage}/>
            </div>)}

          {activeTab === "companies" && (<div>
              <p className="text-sm text-text-2 mb-4">
                Companies that frequently ask problems from this pattern:
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {allCompanies.map(({ name, count }) => (<div key={name} className="flex items-center justify-between rounded-lg border border-border bg-bg-card px-4 py-3">
          ...
          <div className="mt-12 flex items-center justify-between border-t border-border pt-8">
            {prevTopic ? (<Link href={`/topic/${prevTopic.slug}`} className="flex items-center gap-2 text-sm text-text-2 hover:text-primary transition-colors">
                <ArrowLeft className="h-4 w-4"/>
                <span className="font-jakarta">{prevTopic.title}</span>
              </Link>) : (<div />)}
            {nextTopic ? (<Link href={`/topic/${nextTopic.slug}`} className="flex items-center gap-2 text-sm text-text-2 hover:text-primary transition-colors">
