...
function Heatmap({ days, }: {
    ...
    const handleMouseEnter = useCallback((e: React.MouseEvent<SVGRectElement>, cell: (typeof cells)[0]) => {
    ...
    return (<div className="relative">
      ...
      <div className="overflow-x-auto pb-1" style={{ paddingLeft: LABEL_W }}>
        <div className="relative" style={{ width: svgW - LABEL_W, height: 18 }}>
          {monthLabels.map((m, i) => (<span key={`${m.label}-${m.col}-${i}`} className="absolute top-0 text-[10px] font-mono uppercase tracking-widest" style={{
                left: m.col * STEP,
                color: "var(--text-3)",
            }}>
              {m.label}
      ...
      <div className="overflow-x-auto">
        <svg width={svgW} height={svgH} className="block" role="img" aria-label="52-week activity heatmap">
          
          {DAY_LABELS.map((label, idx) => label ? (<text key={label} x={0} y={idx * STEP + CELL - 1} fill="var(--text-3)" fontSize={9} fontFamily="monospace" textAnchor="start">
                {label}
              </text>) : null)}

          
          {cells.map((cell) => (<rect key={cell.date} x={LABEL_W + cell.col * STEP} y={cell.row * STEP} width={CELL} height={CELL} rx={3} fill={getHeatColor(cell.count)} className="heatmap-cell" onMouseEnter={(e) => handleMouseEnter(e, cell)} onMouseLeave={handleMouseLeave}/>))}
        </svg>
      </div>

      
      {tooltip && (<div className="pointer-events-none fixed z-50 -translate-x-1/2 -translate-y-full" style={{ left: tooltip.x, top: tooltip.y - 8 }}>
      ...
      <div className="mt-3 flex items-center justify-end gap-1.5 text-[10px] font-mono text-text-3">
        ...
        {[0, 1, 2, 3, 4].map((lvl) => (<div key={lvl} className="h-[13px] w-[13px] rounded-[3px]" style={{ backgroundColor: `var(--hm-${lvl})` }}/>))}
...
export default function DashboardPage() {
    ...
    return (<AppShell>
      ...
      {streakData.showWarningBanner && (<div className="mb-6 rounded-xl border border-primary/30 bg-primary/10 px-4 py-3 text-sm text-text-1">
          You solved yesterday but not yet today. Keep the streak alive.
        </div>)}

      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (<div key={stat.label} className="rounded-xl border border-border bg-bg-card p-5 transition-all hover:border-border-hover">
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(320px,1fr)]">
        ...
        <div className="rounded-xl border border-border bg-bg-card p-6">
          ...
          <div className="space-y-3">
            {userStats.recentSolved.length === 0 ? (<p className="text-sm text-text-2">No solved problems yet.</p>) : (userStats.recentSolved.map((problem) => (<a key={`${problem.problemId}-${problem.solvedAt}`} href={problem.url} target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-border bg-bg-elevated px-4 py-3 transition-colors hover:border-primary/30">
      ...
      <div className="rounded-xl border border-border bg-bg-card p-6">
        ...
        <div className="space-y-4">
          {allTopics.map((topic) => {
            ...
            return (<div key={topic.slug} className="flex items-center gap-4">
                <span className="w-40 shrink-0 truncate text-sm text-text-2">
                  {topic.title}
                </span>
                <div className="flex-1">
                  <ProgressBar percentage={progress.percentage} size="sm" showLabel completed={progress.completed} total={progress.total}/>
                </div>
              </div>);
        })}
        </div>
