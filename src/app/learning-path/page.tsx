    ...
    return (<AppShell>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        {learningPaths.map((path) => {
            const topicSlugs = getUniqueTopicSlugs(path.modules.flatMap((module) => module.topicSlugs));
            const topics = getPathTopics(topicSlugs);
            const problemIds = Array.from(new Set(topics.flatMap((topic) => topic.patterns.flatMap((pattern) => pattern.problems.map((problem) => problem.id)))));
            const totalPatterns = topics.reduce((sum, topic) => sum + topic.patterns.length, 0);
            ...
            return (<section key={path.id} className="rounded-xl border border-border bg-bg-card p-6">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <h2 className="font-jakarta text-xl font-bold text-text-1">
                    <Link href={`/learning-path/${path.id}`} className="transition-colors hover:text-primary">
                      {path.title}
                    </Link>
                  </h2>
              ...
              <div className="mt-5">
                <ProgressBar percentage={completion} size="md" showLabel completed={solvedCount} total={totalProblems}/>
              </div>
              ...
              <div className="mt-5 rounded-lg border border-border bg-bg-elevated p-3">
                ...
                <ul className="mt-2 space-y-1.5">
                  {path.modules.slice(0, 3).map((module) => (<li key={module.id} className="text-sm text-text-2">
                      {module.title}
                    </li>))}
                </ul>
              </div>
              ...
              <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <Link href={`/learning-path/${path.id}`} className="inline-flex items-center gap-2 rounded-lg border border-border bg-bg-elevated px-4 py-2 text-sm font-jakarta font-semibold text-text-2 transition-colors hover:border-primary/40 hover:text-text-1">
                    View Path
                  </Link>
                  <Link href={nextTopic ? `/topic/${nextTopic.slug}` : "/topics"} className="group inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-jakarta font-bold text-bg-root transition-all hover:bg-primary-dim">
