...
export async function GET() {
    try {
        ...
        const topicsCovered = allTopics.filter((topic) => topic.patterns.some((pattern) => pattern.problems.some((problem) => solvedSet.has(problem.id)))).length;
        const difficultyBreakdown = solvedIds.reduce((acc, problemId) => {
